...
- **修复策略**：多形态分层修复，命中即修；修不了的极端情况原样发出。覆盖的畸形形态：
  1. **结尾 `{}` 取代 `"}`**（最常见）：`{"cmd":"ls","desc":"x{}` → 修复结尾为 `"}`
  2. **缺闭合符**：`{"cmd":"ls"` → 补 `"}`
  3. **对象未闭合就嵌入下一个 `{`**：`{"a":1{"a":1}`（第一个对象没闭合就塞进第二个）→ 删掉多余的 `{` 及之后内容，补 `}` 闭合
  4. **整段重复 / 后掺垃圾**：`{"a":1}{"a":1}` 或 `{"a":1}GARBAGE` → 取第一个完整合法 JSON 对象，丢弃其后内容

  分支 3 用字符串/括号状态机定位多余的 `{`，分支 4 用 `json.loads` 逐位置截断验证，均以 `json.loads` 成功作为最终判据，避免误伤合法 JSON。
...
- 修复覆盖结尾 `{}`、缺闭合符、对象未闭合嵌入 `{`、整段重复/后掺垃圾等已观测形态；遇到未覆盖的全新畸形形态时，混合流式版原样发出（Claude Code 会报错，需重试），缓冲版有重试兜底。

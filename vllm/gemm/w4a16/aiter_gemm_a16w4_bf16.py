...
def awq_gemm_kernel_inner(a_ptr, b_ptr, c_ptr, zeros_ptr, scales_ptr, tile_idx, k_idx, iter_begin, iter_end,
    ...
    if USE_REDUCE_KERNEL:
        ...
        if c_ptr.type.element_ty == tl.float16 or c_ptr.type.element_ty == tl.bfloat16:
            tl.store(c_ptrs, c, mask=c_mask)
        elif not_reduce:
            tl.store(c_ptrs, c, mask=c_mask)
        else:
...
def awq_gemm_kernel_splitk_fused(
    ...
    if pid < total_tiles * SPLITK:
        ...
        acc_out = acc.to(out_ptr.type.element_ty)
        tl.store(out_ptr + output_offsets, acc_out, mask=mask)
...
def awq_gemm_triton_fused_impl(
    ...
    # 最终输出 dtype 跟随输入 (见 awq_gemm_triton_impl 同名注释)
    ...
    final_result = torch.zeros((M, N), dtype=out_dtype, device=input.device)
...
def awq_gemm_triton_impl(input: torch.tensor,
    ...
    # 最终输出 dtype 跟随输入激活值: bf16 模型输出 bf16, fp16 模型输出 fp16。
    # 注意 d_dtype (config D_DTYPE) 是中间 result 缓冲精度 (dp=fp16, splitk=fp32),
    # 与最终输出 dtype 分离 —— splitk 路径中间 fp32 累加, 最后才转 out_dtype,
    # 避免 fp16 中转损失 bf16 精度 (fp32->bf16 一次舍入)。fp16 模型 out_dtype=fp16
    # 行为与原 kernel 完全一致。
    out_dtype = input.dtype
    ...
    if result.ndim == 3:
        ...
        final_result = torch.empty((M, N), dtype=out_dtype, device=input.device)
...
def awq_reduce_and_convert_kernel(
    ...
    output_offsets = offs_m[:, None] * N + offs_n[None, :]
    acc_out = acc.to(output_ptr.type.element_ty)
    tl.store(output_ptr + output_offsets, acc_out, mask=mask)

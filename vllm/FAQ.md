...
| 层级 | 名称 | 代码位置 | 首次出现 | 说明 |
|---|---|---|---|---|
| 引擎层 | **V0 engine**（旧）| `vllm/engine/`、`vllm/core/` | 早期 | 老架构，`LLMEngine` 已在 `#25033` 删除 |
...
| 版本 | V1 engine（引擎层）| MRv2（执行器层）| PagedAttention kernel |
|---|---|---|---|
| v0.6.0 | ❌ 无 | ❌ 无 | ✅ 在 |
| **v0.7.0** | ✅ 引入（`#9289`），默认关 | — | ✅ 在 |
| **v0.8.0** | 默认开（`VLLM_USE_V1=True`）| — | ✅ 在 |
| v0.12.0 | `use_v1` 参数废弃（`#28112`），固化 | — | ✅ 在 |
| **v0.16.0** | 已默认 | ✅ **MRv2 引入**（`#25266`）| ✅ 在 |
| v0.18.0 | 已默认 | 112 commit，成熟中 | ✅ 在 |
| v0.24.0 | 已默认 | 200 commit，能力补齐 | ✅ 在 |
| **v0.25.0** | `#25033` 删除 V0 `LLMEngine` | 220 commit | ❌ **删除**（`d715b3aa1`）|
